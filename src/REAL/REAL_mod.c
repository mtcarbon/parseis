...
int main(int argc, char** argv)
{
    ...
    while (Find_min(ptrig, Nst, Nps) < Maxt0) {
        ...
        if (igrid == 0) {
            ...
            //get time for perfomance measurement 
...
#pragma omp barrier
            // record and report performance measurement
            tdata = omp_get_wtime() - tdata;
            printf("Parallel grid search section 1a calculated in %f secs using %d threads\n", 
                    tdata, omp_get_num_threads());
...
#pragma omp barrier
            // record and report performance measurement
            tdata = omp_get_wtime() - tdata;
            printf("Parallel grid search section 1b calculated in %f secs using %d threads\n", 
                    tdata, omp_get_num_threads());
        }
...
#pragma omp barrier
    // record and report performance measurement
    tdata = omp_get_wtime() - tdata;
    printf("Parallel grid search section 2 calculated in %f secs using %d threads\n", 
            tdata, omp_get_num_threads());
